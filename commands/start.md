...
- **The rest is a path to an existing file**: that file is the intent. Don't offer to save it
  again.
- **The rest is text**: it's a request pasted in. Write it to a temporary file outside the
  ...
  saved yet.
- **Nothing**: ask the engineer to paste the request or name a file, and wait.
...
The spec id will be today's date and the slug (`date -u +%F`, then `-<slug>`). If the current
...
If it exits 1, a spec with that id already exists: propose another slug and try again.

When the intent was a file inside the repository but outside the specs directory, it now exists
twice. Offer to remove the original so the spec directory holds the only copy, and remove it
only if the engineer agrees.
